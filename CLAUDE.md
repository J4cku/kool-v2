...
- Idle dot drop (easter egg): gated by the PostHog feature flag `dot-drop` (fail-closed — no flag, no drop; enable in PostHog → Feature flags), read with the same `useSyncExternalStore(subscribeFeatureFlags, …)` idiom as `brief-form` in `components/Navbar.tsx`. One deliberate exception: it defaults **on** when `NODE_ENV === 'development'`, because PostHog is a no-op without `NEXT_PUBLIC_POSTHOG_KEY` and a pure fail-closed gate would make the effect impossible to see locally; `NODE_ENV` is inlined identically into the server and client bundles, so the server and client snapshots agree and hydration never flips. The flag feeds `useIdle`'s `enabled` argument, so with it off no idle timer and no input listener is registered at all. It gates the drop only — the footer hairline is an SVG (`components/FooterHairline.tsx`) for every visitor either way, and its resting render is pixel-identical to the old `bg-coral scaleY(0.5)` box. Spec: `docs/superpowers/specs/2026-07-24-idle-dot-drop-design.md`
