...
- **Delegate the coding to a Fable 5 subagent.** Hand implementation (writing/editing code) to a subagent running Claude Fable 5 (the Claude 5 top tier; there is no "Opus 5"): `Agent` tool with `model: fable` (agent type `general-purpose`). If Fable is unavailable in the harness, fall back to `model: opus` (Opus 4.8). Give it the plan, the exact files to touch, and the conventions to follow.
...
- **Feature flags now do reach the browser** — this reverses an earlier finding. posthog-js still sends `distinct_id: "$posthog_cookieless"` (the default state, given `opt_out_capturing_by_default: true` + `cookieless_mode: 'on_reject'`), but PostHog now evaluates flags for it: re-verified against koolstudio.pl on 2026-08-27, `isFeatureEnabled('brief-form')` is `true` before any consent choice and the /kontakt CTA renders for every visitor. Flags stay fail-closed by design in `lib/analytics.ts` (`false` until the deferred posthog init resolves), so anything gated on one is invisible for the first ~1–3s of a page — fine for a CTA, not for above-the-fold content. Re-check with a real browser before trusting a flag gate again
- Idle dot drop (easter egg): ungated, for the reason above — a `dot-drop` flag was created, wired up and then removed. It ships to everyone, suppressed only by `prefers-reduced-motion`, an open menu, an open dialog (`[aria-modal="true"]`), a hidden tab, or a missing footer line. The footer hairline is an SVG (`components/FooterHairline.tsx`) whose resting render is pixel-identical to the old `bg-coral scaleY(0.5)` box (0 differing pixels at DPR 1 and 2). Spec: `docs/superpowers/specs/2026-07-24-idle-dot-drop-design.md`
- `skipTrailingSlashRedirect` removes Next's sitewide slash normalization, so `proxy.ts` restores the trailing-slash 308 for page routes itself; `proxy.ts` also 308s unprefixed paths to `/pl/...` itself (in the same single redirect) because next-intl's built-in unprefixed→locale redirect is a 307 (temporary), which made Google keep the pre-i18n unprefixed URLs as its chosen canonicals (GSC "Duplicate, Google chose different canonical")
...
- Meta Pixel: dataset **KOOL Studio | Website**, pixel ID `2041137253179699`, hardcoded in `lib/site.ts` as `META_PIXEL_ID` (a pixel ID is public by construction; `NEXT_PUBLIC_META_PIXEL_ID` overrides it, and `""` disables the pixel — `??`, so an empty value is the kill switch, not "unset")
- The Pixel runs **only after "akceptuję"** — it reuses PostHog's single consent store (`consentStatus`/`subscribeConsentStatus`), because `privacy.adsBody` promises in both locales that ad tooling is not loaded without consent. No second banner, no `<noscript>` fallback pixel (it would fire before any choice)
- `PageView` is driven by `components/MetaPixel.tsx` on pathname change, **not** by Meta's snippet: `lib/meta-pixel.ts` installs the base code minus its trailing `fbq('track','PageView')`, since every in-site navigation is client-side and keeping both would double-count the landing page. The component reads `usePathname` from `next/navigation` (not `@/i18n/navigation`, which strips the locale prefix and would hide a pl→en switch)
- `Lead` fires **only** on `status: 'success'` from `submitBrief` — i.e. Resend confirmed delivery. Spam, validation failures and the mailto fallback produce nothing
- Dedup contract: `submitBrief` mints `metaEventId` (uuid) server-side and returns it in the action state; the browser sends it as `eventID`, CAPI as `event_id`, and Meta collapses the pair into one conversion. Both sides also send `content_name: 'project-brief'`
- CAPI (`lib/meta-capi.ts`, framework-free so `tests/meta-capi.test.ts` can run it under `node --experimental-strip-types`) runs inside `after()` from `next/server`, gated on the `marketingConsent` hidden input **and** `META_CAPI_ACCESS_TOKEN`; `META_TEST_EVENT_CODE` routes events to Events Manager → Test Events. Only a hashed e-mail + first name, IP, user agent, `_fbp`/`_fbc` and the project type leave the server. A Lead of any kind also requires `RESEND_API_KEY` in production — without it the form never reaches `success`
- `pnpm dev` never loads the pixel (`NODE_ENV === 'development'` guard in `MetaPixel.tsx`); vitest runs as `test` so the consent-gate test exercises the real path, and Vercel previews build as production on purpose. Spec: `docs/superpowers/specs/2026-08-27-meta-pixel-capi-design.md`
