...
- Contact/brief form: always-on modal on /kontakt (`components/kontakt/BriefModal.tsx` — trigger button under the hero heading, deep-linkable via `#brief`; controlled form in `components/kontakt/BriefForm.tsx` + server action in `app/[locale]/kontakt/actions.ts`). The page owns the draft and stable anti-spam timestamp across close/reopen, and clears them only after confirmed delivery. Validation/spam logic lives in `lib/brief.ts` (tested in `tests/brief-validation.test.ts`); delivery uses Resend (`RESEND_API_KEY`, `BRIEF_FROM_EMAIL`, `BRIEF_TO_EMAIL`) with a customer receipt in the form's locale, and falls back to a prefilled mailto when unconfigured or on delivery failure.
