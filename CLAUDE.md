...
- **Feature flags do not reach the browser on this site.** posthog-js sends `distinct_id: "$posthog_cookieless"` — the default state, given `opt_out_capturing_by_default: true` + `cookieless_mode: 'on_reject'` — and PostHog answers `flags: {}`, so `isFeatureEnabled()` returns `undefined` for every flag. Verified against koolstudio.pl itself: before consent, after accepting cookies, and after a reload. The same request with an ordinary `distinct_id` returns the flags fine, so the key, token and `/dot` proxy are all healthy. **Consequence: the `brief-form` gate above is dead in production** — the /kontakt CTA is hidden from every visitor. Flag evaluation needs no personal data, so the fix is to let flags load independently of consent; until then, do not gate anything user-visible on a PostHog flag
