...
Client review supersedes the static inline 16:9 hero and static manifesto described in Task 3. The Studio route must reuse `ProjectHero` with `/images/studio/team.webp`, render an `h-screen` spacer, and place all beige content plus `FooterBanner showMarquee={false}` inside a `relative z-10 bg-beige` wrapper. Replace the static manifesto heading with the existing homepage `animate-marquee` pattern: four repeated copies in one semantic `h1`, with only the first copy exposed to assistive technology; add `motion-reduce:animate-none` so the perpetual animation stops when the visitor prefers reduced motion. The localized press-section heading uses Poppins font weight `700`, explicitly superseding the older Task 3 sample's `font-[900]`; publication-card heading weights do not change. The Studio introduction reuses the established Oferta body treatment—`max-w-[610px] font-[400] leading-[1.5] text-dark/80` with `fontSize: 'clamp(15px, 1.5vw, 20px)'`—superseding the larger original Task 3 sample while preserving its copy, width, and position. The marquee uses `pt-16 pb-8 md:pt-24 md:pb-12`, and the intro uses `pt-10 pb-20 md:pt-14 md:pb-28 lg:pt-20 lg:pb-36`; these asymmetric classes supersede the older symmetric `py-*` sample, tighten the gap between sections, and preserve the intro's lower rhythm. The press header layout, links, and responsive 1/2/3-column grid remain unchanged.
